...
const validateBookingDatafn = async (BookingFiledIds, packageId, addons, activityId, isSuvSelected, suvCount = 0) => {
    let totalCalculatedAmount = 0;
    let detailCalculatedAmount = [];
    let extraDurationFieldPrice = 0;
    let totalExtraMember = 0;
    ...
    detailCalculatedAmount.push({ label: "Base Package", amount: packageInfo.price });
    ...
    for (const userInput of BookingFiledIds) {
        ...
        if (!fieldDefinition) throw new Error(`Invalid booking field ID: ${userInput.id}`);
        if (fieldDefinition.min && userInput.value < fieldDefinition.min) throw new Error(`${fieldDefinition.name} must be at least ${fieldDefinition.min}`);
        if (fieldDefinition.max && userInput.value > fieldDefinition.max) throw new Error(`${fieldDefinition.name} cannot exceed ${fieldDefinition.max}`);
        if (fieldDefinition.duration && (userInput.value < fieldDefinition.duration || userInput.value % fieldDefinition.duration != 0)) {
            throw new Error(`${fieldDefinition.name} must be at least ${fieldDefinition.duration} or in proper fraction`);
        }
        ...
        if (valueToCalculateOn) {
            if (fieldDefinition?.duration) {
                ...
                newPrice = (totalExtraMinX - 1) * fieldDefinition?.price;
                extraDurationFieldPrice = totalExtraMinX * fieldDefinition?.price;
                detailCalculatedAmount.push({ label: `${fieldDefinition.name} (Duration)`, amount: extraDurationFieldPrice });
            } else {
                minValue = fieldDefinition?.min > 0 ? fieldDefinition.min : 1;
                let extraMember = valueToCalculateOn - fieldDefinition?.min;
                totalExtraMember += extraMember / minValue;
                totalRequiredseat = fieldDefinition?.seat ? (fieldDefinition?.seat * (extraMember + fieldDefinition?.min)) : totalRequiredseat + extraMember + fieldDefinition?.min;
                minQuantityPrice += totalExtraMember * fieldDefinition.price;
                detailCalculatedAmount.push({ label: fieldDefinition.name, amount: totalExtraMember * fieldDefinition.price });
            }
        }
    }
    ...
    for (const addon of addons) {
        ...
        if (found) {
            addonPrice += found.price * (addon.quantity || 1);
            detailCalculatedAmount.push({ label: `Addon: ${found.title}`, amount: found.price * (addon.quantity || 1) });
        }
    }
    ...
    if (isSuvSelected && suvCount > 0) {
        const suvInfo = await Activity_Malik.findOne({ _id: activityId }).select("PrivateSUV name");
        if (!suvInfo?.PrivateSUV?.available) throw new Error("SUV is not available");

        const suvCost = suvInfo.PrivateSUV.fee * suvCount;
        totalCalculatedAmount += suvCost;
        detailCalculatedAmount.push({ label: `Private SUV (Qty: ${suvCount})`, amount: suvCost });
    }

    return { calculatedAmount: totalCalculatedAmount, breakdown: detailCalculatedAmount };
};
...
export const createOrder = async (req, res) => {
    try {
        const { 
            BookingFiledIds, packageId, activityId, isSuvSelected, suvCount,
            addons, totalAmount, date, time, bookingId,
            paymentMethod, guestDetails, notes
        } = req.body;

        // 1. Calculate & Validate
        const { calculatedAmount, breakdown } = await validateBookingDatafn(
            BookingFiledIds, packageId, addons, activityId, isSuvSelected, suvCount
        );

        // 2. Prepare Structured Object
        const bookingData = {
            user: req.user?._id || "68be96f7d6c63cf73830142a",
            activity: activityId,
            packageId,
            date: new Date(date),
            timeSlot: time,
            ...
            extras: {
                isSuvSelected,
                suvCount: isSuvSelected ? suvCount : 0,
                notes: notes
            },
            
            paymentMethod,
            totalAmount: calculatedAmount,
            amountBreakdown: breakdown, // Ensure your validate function adds 'quantity' to this
            status: "pending",
        ...
        if (paymentMethod === "pay_now") {
            ...
            return res.status(200).json({ success: true, clientSecret: paymentIntent.client_secret, bookingId: booking._id });
        } else {
            const booking = await Booking.create(bookingData);
            return res.status(200).json({ success: true, message: "Booking confirmed (Pay Later)", data: booking });
        }
